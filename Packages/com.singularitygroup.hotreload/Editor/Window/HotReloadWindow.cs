...
namespace SingularityGroup.HotReload.Editor {
    class HotReloadWindow : EditorWindow {
        ...
        void OnEnable() {
            ...
            if(HotReloadPrefs.ShouldDoAutoRefreshFixup) {
                if (new AutoRefreshSettingChecker().IsApplied()) {
                    EditorPrefs.SetInt(AutoRefreshSettingChecker.autoRefreshKey, 0);
                }
                HotReloadPrefs.ShouldDoAutoRefreshFixup = false;
            }
#endif
        }
        ...
        void OnGUI() {
            ...
            if (AutoRefreshSettingChecker.AutoRefreshPreference == 2 && !Application.isPlaying && runTab.Running) {
                EditorGUILayout.LabelField("Auto refresh is enabled outside of Playmode. If you plan to use Hot Reload in edit mode make sure to disable auto refresh completely in the preferences", HotReloadWindowStyles.WrapStyle);
        ...
        void RenderRateApp() {
            if (HotReloadPrefs.RateAppShown || !PackageConst.IsAssetStoreBuild) {
                return;
            }
            ...
            using (new EditorGUILayout.HorizontalScope()) {
                if (GUILayout.Button("Yes")) {
                    var openedUrl = EditorUtility.DisplayDialog("Rate Hot Reload", "Thank you for using Hot Reload!\n\nPlease consider leaving a review on the Asset Store to support us.", "Open in browser", "Cancel");
                    if (openedUrl) {
